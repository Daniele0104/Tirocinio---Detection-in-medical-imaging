...
def is_main_process():
    """Return True if this is the main process (works for single-GPU too)."""
...
class HGNetv2(nn.Module):
    ...
    def __init__(self,
        ...
        if pretrained:
            ...
            def is_main_process():
                """Return True if this is the main process (works in single-GPU too)."""
                return not torch.distributed.is_initialized() or torch.distributed.get_rank() == 0

            try:
                if name in ['Atto', 'Femto', 'Pico']:
                    model_path = os.path.join(local_model_dir, 'PPHGNetV2_B0_stage1.pth')
                else:
